...
public class ReservationSystem extends JFrame implements ActionListener {
    ...
    private JLabel usernameLabel, passwordLabel, titleLabel, nameLabel, emailLabel, phoneLabel, dateLabel, sourceLabel, destinationLabel, trainNumberLabel, classLabel, pnrLabel;
    private JTextField usernameField, nameField, emailField, phoneField, sourceField, destinationField, trainNumberField, pnrField;
    private JPasswordField passwordField;
    private JButton loginButton, reserveButton, cancelButton;
    private JTextArea reservationArea;
    private JComboBox<String> dateBox, classBox;
    private ArrayList<Reservation> reservations = new ArrayList<>();
    private User currentUser;
    private PNRNumber pnrGenerator;
    ...
    public ReservationSystem() {
        ...
        pnrLabel.setBounds(80, 490, 100, 20);
        add(pnrLabel);

        pnrField = new JTextField();
        pnrField.setBounds(180, 490, 150, 20);
        add(pnrField);

        reservationArea = new JTextArea();
        reservationArea.setEditable(false);
        JScrollPane scrollPane = new JScrollPane(reservationArea);
        scrollPane.setBounds(80, 530, 300, 80);
        add(scrollPane);
        setVisible(true);

        pnrGenerator = new PNRNumber();
    }
        ...
        if (e.getSource() == loginButton) {
            ...
            if (currentUser == null) {
                ...
                if (pnrNumber.isEmpty()) {
                    pnrNumber = pnrGenerator.generatePNR();
                }

                Reservation reservation = new Reservation(name, date, source, destination, trainNumber, className, pnrNumber);
            ...
            if (currentUser == null) {
                ...
                for (Reservation r : reservations) {
                    if (r.getName().equals(name) && r.getDate().equals(date) && r.getSource().equals(source) && r.getDestination().equals(destination) && r.getPNRNumber().equals(pnrNumber)) {
    ...
    class Reservation {
        ...
        public Reservation(String name, String date, String source, String destination, String trainNumber, String className, String pnrNumber) {
            this.name = name;
            this.date = date;
            this.source = source;
